...
void Main()
{
	...
	*/
	...
	InsertEvent(Guid.NewGuid(), $"ChangeAssessment-Testing-Scenario 02-{now.ToString("yyyyMMdd_HHmmss")}",eventStart, eventEnd);
	
	bool continuetest = true;
	using (var client = new HttpClient())
	{
		Console.WriteLine("Starting: Scenario 2, Part 1");
	...
	if (continuetest)
	{
		...
		Console.WriteLine($"Starting: Scenario 2, Part 2. Sleeping for {sleepTime.TotalMinutes} minutes");
		...
		using (var client1 = new HttpClient())
		{
			...
			if (status_code == expected_status_code)
			{
				...
				if (string.IsNullOrEmpty(results.exceptionRequestUrl.ToString()))
				{
					Console.WriteLine($"[Failed] Scenario 2, part 2 GET: expected_url should be different than (null, empty)");
					continuetest = false;
				}
			}
			else
	...
	if (continuetest)
	{
		...
		TimeSpan sleepTime = eventEnd.AddSeconds(5.0) - DateTime.UtcNow;
		Console.WriteLine($"Starting: Scenario 2, Part 3. Sleeping for {sleepTime.TotalMinutes} minutes");
...
public class Ev2DeploymentAssessment
{
	...
	public IList<Ev2DeploymentAssessmentDetail> Ev2DeploymentAssessmentDetails { get; internal set; }
...
public class Ev2DeploymentAssessmentDetail
{
	...
	public IList<Guid> ExceptionsReviewed { get; internal set; }
}
