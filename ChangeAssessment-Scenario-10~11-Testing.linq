...
void Main()
{
	...
	*/
	...
	// -- Deployment Allowed to Continue: Pending while the exception is being created; NO, after the exception is declined.

	// Let's have an event
	DateTime now = DateTime.UtcNow;
	DateTime eventStart = now.AddMinutes(-1.0);
	DateTime eventEnd = now.AddMinutes(1.0);
	InsertEvent(Guid.NewGuid(), $"ChangeAssessment-Testing-Scenario 10~11-{now.ToString("yyyyMMdd_HHmmss")}", eventStart, eventEnd);

	Ev2DeploymentAssessment deploymentAssessment = null;
	bool continuetest = true;
	using (var client = new HttpClient())
	{
		Console.WriteLine("Starting: Scenario 10, Part 1: Creating Deployment");
		...
		if (status_code == expected_status_code)
		{
			...
			{
				Console.WriteLine($"[Failed] Scenario 10, part 1 POST: expected_url should be different than (null, empty)");
			...
			{
				Console.WriteLine($"[Failed] Scenario 10, part 1 POST: At least one AllowedToDeploy should be false");
				Console.WriteLine(JsonConvert.SerializeObject(deploymentAssessment, Newtonsoft.Json.Formatting.Indented));
				continuetest = false;
			}
			...
			{
				Console.WriteLine($"[Failed] Scenario 10, part 1 POST: At least one detail should not have an exception reviewed");
				Console.WriteLine(JsonConvert.SerializeObject(deploymentAssessment, Newtonsoft.Json.Formatting.Indented));
				continuetest = false;
			}
		}
		...
		{
			Console.WriteLine($"[Failed] Scenario 10, part 1 POST: Expected status code '200' != {status_code} ");
			continuetest = false;
		}
	}
	...
	if (continuetest)
	{
		using (var client = new HttpClient())
		{
			...
			{
				Console.WriteLine($"[Failed] Scenario 10, part 1A GET (retrieving the ChageAssessment created to validate it): Expected status code '200' != {status_code} ");
				continuetest = false;
			}
		}
	}
	...
	if (continuetest)
	{
		...
		using (var client = new HttpClient())
		{
			Console.WriteLine("Starting: Scenario 10, Part 2: Create ExceptionRequest from DeploymentUrl");
			...
			if (status_code == expected_status_code)
			{
				...
				{
					Console.WriteLine($"[Failed] Scenario 10, part 2 POST: DeploymentOfOrgin should match RolloutId");
			...
			{
				Console.WriteLine($"[Failed] Scenario 10, part 2 POST: Expected status code '200' != {status_code} ");
				string json = response.Content.ReadAsStringAsync().Result;
	...
	if (continuetest)
	{
		...
		int BeginDifferenceMilliseconds = System.Convert.ToInt32(System.Math.Abs((ExceptionBeginsOn - eventStart).TotalMilliseconds));
		int EndDifferenceMilliseconds = System.Convert.ToInt32(System.Math.Abs((ExceptionEndsOn - eventEnd).TotalMilliseconds));
		if (BeginDifferenceMilliseconds > 500 || EndDifferenceMilliseconds > 500)
		{
			...
			Console.WriteLine($"LatestEventEnd: {LatestEventEnd:O}, ExceptionEndsOn: {ExceptionEndsOn:O}. DeltaEnd: {System.Math.Abs((ExceptionEndsOn - eventEnd).TotalMilliseconds)}.");
		}
	}
	...
	if (continuetest)
	{
		...
		Console.WriteLine($"Starting: Scenario 10, Part 3. Checking deployment is still pending while the exception gets processed");
		...
		using (var client1 = new HttpClient())
		{
			...
			if (status_code == expected_status_code)
			{
				...
				{
					Console.WriteLine($"[Failed] Scenario 10, part 3 GET: expected_url should be different than (null, empty)");
					continuetest = false;
				}
			}
			else
			{
				Console.WriteLine($"[Failed] Scenario 10, part 3 GET: Expected status code '200' != {status_code} ");
				continuetest = false;
			}
		}
	}
	...
	if (continuetest)
	{
		...
		using (var client = new HttpClient())
		{
			Console.WriteLine("Starting: Scenario 10, Part4: Submit ExceptionRequest for Revision");
			int expected_status_code = 200;

			exceptionrequest.businessJustification = $"BusinessJustification: ChangeAssessment-Testing-Scenario 10~11-{now.ToString("yyyyMMdd_HHmmss")}";
			exceptionrequest.title = $"Title: ChangeAssessment-Testing-Scenario 10~11-{now.ToString("yyyyMMdd_HHmmss")}";
			...
			if (status_code != expected_status_code)
			{
				Console.WriteLine($"[Failed] Scenario 10, part 4 PUT: Expected status code '200' != {status_code} ");
				string json = response.Content.ReadAsStringAsync().Result;
				Console.WriteLine(json);
				continuetest = false;
			}
	...
	if (continuetest)
	{
		...
		using (var client = new HttpClient())
		{
			...
			{
				Console.WriteLine($"[Failed] Scenario 10, part 4B GET: Expected status code '200' != {status_code} ");
				continuetest = false;
			}
		}
	}
	...
	if (continuetest)
	{
		...
		using (var client = new HttpClient())
		{
			Console.WriteLine("Starting: Scenario 10, Part 5: Decline ExceptionRequest");
			int expected_status_code = 200;

			exceptionrequest.approvedBy = "jujofre@microsoft.com";
			exceptionrequest.status = "Declined";

			dynamic approverNote = new JObject();
			approverNote.approverEmail = "jujofre@microsoft.com";
			approverNote.notes = "Declined per scenario 10 requirement.";
			exceptionrequest.approverNotes = new JArray();
			exceptionrequest.approverNotes.Add(approverNote);
			...
			if (status_code != expected_status_code)
			{
				Console.WriteLine($"[Failed] Scenario 10, part 5 PUT: Expected status code '200' != {status_code} ");
				string json = response.Content.ReadAsStringAsync().Result;
				Console.WriteLine(json);
				continuetest = false;
			}
	...
	if (continuetest)
	{
		// Let's check deployment is blocked once exception was declined
		Console.WriteLine($"Starting: Scenario 10, Part 6. Checking deployment is blocked");
		...
		using (var client1 = new HttpClient())
		{
			...
			if (status_code == expected_status_code)
			{
				...
				{
					Console.WriteLine($"[Failed] Scenario 10, part 6 POST: expected_url should be (null, empty)");
				...
				{
					Console.WriteLine($"[Failed] Scenario 10, part 6 GET: AllowedToDeploy should be all false");
					Console.WriteLine(JsonConvert.SerializeObject(results, Newtonsoft.Json.Formatting.Indented));
					continuetest = false;
				}
				...
				{
					Console.WriteLine($"[Failed] Scenario 10, part 6 GET: All details should have an exception reviewed");
					Console.WriteLine(JsonConvert.SerializeObject(results, Newtonsoft.Json.Formatting.Indented));
					continuetest = false;
				}
			}
			...
			{
				Console.WriteLine($"[Failed] Scenario 10, part 6 GET: Expected status code '200' != {status_code} ");
				continuetest = false;
			}
		}
	}
	...
	if (continuetest)
	{
		...
		// -- Exception Scenario:             (reactive Exception declined from previous scenario)
		// -- Deployment Allowed to Continue: No

		// Let's start another deployment half way through the event
		Console.WriteLine($"Starting: Scenario 11, Part 1. ");
		...
		using (var client = new HttpClient())
		{
			...
			if (status_code == expected_status_code)
			{
				...
				if (!string.IsNullOrEmpty(results.ExceptionRequestUrl))
				{
					Console.WriteLine($"[Failed] Scenario 11, part 1 POST: expected_url should be (null, empty)");
					Console.WriteLine(JsonConvert.SerializeObject(results, Newtonsoft.Json.Formatting.Indented));
					continuetest = false;
				}
				...
				if (!results.Ev2DeploymentAssessmentDetails.All(d => !d.AllowedToDeploy))
				{
					Console.WriteLine($"[Failed] Scenario 11, part 1 POST: AllowedToDeploy should be all false");
					Console.WriteLine(JsonConvert.SerializeObject(results, Newtonsoft.Json.Formatting.Indented));
					continuetest = false;
				}
				...
				if (!results.Ev2DeploymentAssessmentDetails.All(d => d.ExceptionsReviewed.Length > 0))
				{
					Console.WriteLine($"[Failed] Scenario 11, part 1 POST: All details should have an exception reviewed");
					Console.WriteLine(JsonConvert.SerializeObject(results, Newtonsoft.Json.Formatting.Indented));
					continuetest = false;
				}
			}
			...
			{
				Console.WriteLine($"[Failed] Scenario 11, part 1 POST: Expected status code '200' != {status_code} ");
				continuetest = false;
			}
		}
