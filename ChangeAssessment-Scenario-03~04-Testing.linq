...
void Main()
{
	...
	*/
	...
	// -- Exception Scenario:             Proactive Exception approved
	...
	InsertEvent(Guid.NewGuid(), $"ChangeAssessment-Testing-Scenario 03~04-{now.ToString("yyyyMMdd_HHmmss")}", eventStart, eventEnd);
	
	// Let's have a proactive exception to cover the event
	Guid exceptionRequestId = Guid.NewGuid();
	DateTime requestDate = now;
	InsertExceptionRequest(exceptionRequestId, requestDate, eventStart, eventEnd, "Approved", requestDate);
	
	bool continuetest = true;
	using (var client = new HttpClient())
	{
		Console.WriteLine("Starting: Scenario 3, Part 1");
		...
		if (status_code == expected_status_code)
		{
			...
			{
				Console.WriteLine($"[Failed] Scenario 3, part 1 POST: AllowedToDeploy should be all true");
				Console.WriteLine(JsonConvert.SerializeObject(results, Newtonsoft.Json.Formatting.Indented));
				continuetest = false;
			}
			...
			{
				Console.WriteLine($"[Failed] Scenario 3, part 1 POST: All details should have an exception reviewed");
				Console.WriteLine(JsonConvert.SerializeObject(results, Newtonsoft.Json.Formatting.Indented));
				continuetest = false;
			}
		}
		...
		{
			Console.WriteLine($"[Failed] Scenario 3, part 1 POST: Expected status code '200' != {status_code} ");
			continuetest = false;
		}
	}
	...
	if (continuetest)
	{
		...
		// -- Exception Source:               N/A
		// -- Exception Scenario:             (Proactive Exception approved from previous scenario)
		// -- Deployment Allowed to Continue: Yes
		...
		Console.WriteLine($"Starting: Scenario 4, Part 1. Sleeping for {sleepTime.TotalMinutes} minutes");
		...
		using (var client = new HttpClient())
		{
			...
			if (status_code == expected_status_code)
			{
				...
				{
					Console.WriteLine($"[Failed] Scenario 3, part 2 POST: expected_url should (null, empty)");
					continuetest = false;
				}
			}
			else
			{
				Console.WriteLine($"[Failed] Scenario 3, part 2 POST: Expected status code '200' != {status_code} ");
				continuetest = false;
			}
		}
